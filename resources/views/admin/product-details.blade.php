@extends('admin.layout.master')
@section('title','Product Details')
@section('heading')
{{Str::ucfirst($product->categories->name)}}
@stop

 @section('content')




     <!-- card code  -->
     <div class="container card-style">
        <div class=" row ">

               <div class="col-lg-4 col-md-6 col-12  d-flex align-items-stretch mb-3 mx-auto">
                    <div class="card h-auto" style="width: 18rem;" >

                        <img src="/images-uploaded/products/{{$product->photo}}" class="card-img-top img-fluid  " alt="product-pic" >

                        <div class="card-body flex-fill text-center ">
                           <h5 class="card-header  "> Name</h5>
                              <p class="card-text ellipsis ">{{$product->name}}</p>
                            <h5 class="card-header"> Description</h5>
                              <p class="card-text ellipsis ">{{$product->description}}</p>
                            <h5 class="card-header "> Price</h5>
                              <p class="card-text ellipsis ">{{$product->price}}</p>

                        </div>
                    </div>
                </div>

       </div>
       </div>
    @endsection
